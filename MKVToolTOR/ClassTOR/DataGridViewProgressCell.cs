...
namespace TORServices.FormsTor
{
    class DataGridViewProgressCell : DataGridViewImageCell
    {
            ...
            DataGridViewPaintParts paintParts)
        {
            ...
            double progressVal = Convert.ToInt32(value);

            double percentage = ((double)progressVal / 100.0d); // Need to convert to float before division; otherwise C# returns int which is 0 for anything but 100%.
            ...
            if (percentage >= 0.0)
            {
                ...
                //Draw text
                g.DrawString(progressVal.ToString("N2") + " %", cellStyle.Font, foreColorBrush, posX, posY);
            }
            else
            {
                ...
                {
                    g.DrawString(progressVal.ToString("N2") + " %", cellStyle.Font, new SolidBrush(cellStyle.SelectionForeColor), posX, posX);
                }
                else
                {
                    g.DrawString(progressVal.ToString("N2") + " %", cellStyle.Font, foreColorBrush,posX, posY);
